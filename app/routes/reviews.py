...
@router.get("/", response_model=List[schemas.ReviewWithParent])
def read_reviews(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    return crud.get_reviews(db, skip=skip, limit=limit)

@router.get("/by_parent", response_model=List[schemas.Review])
def read_reviews_by_parent(db: Session = Depends(get_db), user: dict = Depends(get_current_parent)):
    return crud.get_reviews_by_parent(db, user["id"])

@router.put("/{review_id}", response_model=schemas.Review)
def update_review(review_id: int, review_update: schemas.ReviewUpdate, db: Session = Depends(get_db), user: dict = Depends(get_current_parent)):
